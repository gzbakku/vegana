...
async function init(){
  ...
  .on('change',async (path)=>{
    ...
    if(false){
      ...
      // console.log(JSON.stringify(kc,null,1));
      console.log(kc);
      console.log("\n");
    }

    if(file_type === "js" || file_type === "json"){
      blocked = true;
      await transform.file(path,true);
      blocked = false;
    }
    ...
    if(file_type === "js" || file_type === "json"){
      if(location === "ui"){
        await compile.appModule(lazy_parent.type,parents,null,true);
      }
      ...
      if(location === "ui"){
        await workers.compile_lazy_sass(lazy_parent,parents);
      }
      if(sass_type === "tree"){
        ...
        if(location === "ui" || lazy_parent.type === "app"){
          ...
          return await workers.compile_lazy_sass(lazy_parent,parents);
        }
  ...
  .on('change',async (path)=>{
    path = io.clean_path(path);
    // console.log({js_path:path});
    if(
      path.includes("js/pages/") ||
      path.includes("js/ui/") ||
      path.includes("js/globals/") ||
      path.includes("js/bundle.js") ||
      path.includes("js/wasm/")
    ){
      common.tell(`internal module updated => ${path}`);
      return;
    }
    // console.log({path:path});
    common.tell('js updated ./js');
    socket.reload();
  });
  ...
  .on('change',async (path)=>{
    common.tell('vegana web engine updated');
    let compileCheck = await compile.bundle();
    socket.reload();
  });

  //static
  let location_static = currentDirectory + 'static/';
  chokidar.watch(location_static)
  .on('change',async (path)=>{
  ...
  //static server
  let location_static_server = currentDirectory + 'static_server.js';
  ...
  //static server
  let location_static_server_config = currentDirectory + 'static_server_config.json';
